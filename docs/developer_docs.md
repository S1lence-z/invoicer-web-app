...
- **Key Contents:**
    ...
    - `Interfaces/`: Defines interfaces for domain services or abstractions needed _within_ the domain itself (`IInvoiceNumberGenerator.cs`, `IInvoiceNumberParser.cs`)
...
- **Purpose:** Currently serves multiple roles:
    ...
    2.  **Composition Root:** Configures dependency injection, middleware, and application startup (`Program.cs`).
- **Key Contents:**
